...
Списки приходят таблицей: ФИО, институт, группа, дата рождения, б/вн.
Загружает их `backend/tools/import_payers.py`. Порядок колонок неважен —
они ищутся по названиям, потому что каждый год таблицу собирают заново.

Курс отдельно указывать не нужно: первая цифра кода группы и есть курс,
из него считается год поступления, дальше курс растёт сам каждое 1 сентября.

### 1. Положить файл в контейнер

```bash
docker cp 2026.xlsx profpay-backend:/tmp/spisok.xlsx
```

### 2. Посмотреть, что получится

```bash
docker compose -f docker-compose.prod.yml exec backend python -m backend.tools.import_payers /tmp/spisok.xlsx
```

Этот запуск ничего не пишет. В отчёте будет разбивка по институтам, курсам
и форме оплаты, а отдельными списками — строки, которые требуют внимания:

| В отчёте | Что это значит |
|---|---|
| «код группы не разобран» | в ячейке `?`, число или пусто — запись загрузится без группы и курса |
| «дата рождения неправдоподобна» | Excel подставил текущий год вместо года рождения — поле останется пустым |
| «курс не укладывается в срок обучения» | пятикурсник не бывает на четырёхлетней программе, уровень поднят до специалитета |
| «попадут сразу в архив» | по этим данным человек уже выпустился и пропадёт из списков — проверьте курс |
| «нет в справочнике» | такого института нет, см. шаг 3 |

Сверьте числа по институтам с таблицей. Не сходится — правьте Excel
и повторяйте этот шаг, он безопасен.

### 3. Загрузить

```bash
docker compose -f docker-compose.prod.yml exec backend python -m backend.tools.import_payers /tmp/spisok.xlsx --apply
```

Если в отчёте были незнакомые институты и вы хотите завести их сразу,
добавьте `--create-faculties`. Без флага такие записи загрузятся без
института, и его можно проставить позже в карточке.

Загрузка идёт одной транзакцией: либо все записи, либо ни одной. Повторный
запуск того же файла ничего не задваивает — совпадения по ФИО пропускаются,
поэтому дослать файл с добавленными строками можно тем же способом.

### 4. Убрать файл

В нём персональные данные, в контейнере ему делать нечего:

```bash
docker compose -f docker-compose.prod.yml exec backend rm /tmp/spisok.xlsx
```

Дальше по отчёту допроверьте в интерфейсе те несколько записей, что были
помечены: поиск по фамилии, «Изменить», проставить группу или дату.
