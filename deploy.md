...
Дальше допроверьте записи, которые отчёт пометил. Держать его под рукой
не нужно: в «Плательщиках» есть галочка **«Только неполные»** — она собирает
все записи, где не хватает группы, года поступления, деректората или даты
рождения. В строке такие помечены значком «неполные», а подсказка на нём
говорит, чего именно не хватает.

Начните с тех, у кого нет группы: без неё не считается курс, и запись
не уйдёт в архив сама — останется в списках и после выпуска.

---

## Снять автоплатежи бюджетников

Делается **один раз** при обновлении с версии, где бюджетнику записывался
платёж на сумму взноса за год. От этого отказались: профком денег
с бюджетника не получает, и такие записи завышали «Собрано средств».

Сначала вхолостую — скрипт только покажет, сколько найдено:

```bash
docker compose -f docker-compose.prod.yml exec backend python -m backend.tools.clear_stipend_payments
```

Число должно совпасть с количеством бюджетников. Тогда удаляйте:

```bash
docker compose -f docker-compose.prod.yml exec backend python -m backend.tools.clear_stipend_payments --apply
```

Трогает только платежи с отметкой «Удержание из стипендии» — те, что завела
система. Внесённые руками не затрагиваются, даже если их вносили бюджетнику.
После этого бюджетники остаются в статусе «Оплачено», а «Собрано средств»
показывает только настоящие деньги с платников.
