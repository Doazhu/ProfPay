/*
 * AeroShards — фоновая графика для страницы входа.
 *
 * Компонент взят как есть из React Bits (@react-bits/AeroShards-JS-CSS,
 * https://reactbits.dev) и не редактируется: обновлять его проще заменой
 * файла целиком. Рисует на WebGPU через пакет vgpu.
 *
 * Наружу его напрямую не подключают — только через LoginBackground.tsx,
 * который проверяет поддержку WebGPU, уважает prefers-reduced-motion и
 * грузит этот файл отдельным куском, чтобы вход открывался сразу.
 */
